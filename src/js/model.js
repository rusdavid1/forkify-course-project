...
import { AJAXCall, getJSON, sendJSON } from './helpers';
...
export const loadSearchResults = async function (q) {
  try {
    const { data } = await AJAXCall(`${API_URL}?search=${q}&key=${API_KEY}`);
    const { recipes } = data;
  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingArr.length !== 3)
          throw new Error(`Invalid ingredients input. Check the format`);
